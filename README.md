...
| 注意事项 | 说明 |
|---------|------|
| **端口占用** | 确保以下端口未被占用：8080、3000、9090、3100、9000、5432、6379 |
| **内存要求** | 至少 8GB 内存，否则 Docker 可能无法正常运行 |
| **首次启动** | 首次启动需要下载 Docker 镜像，请保持网络连接（约 2-5 分钟）|
| **数据持久化** | 所有数据存储在 Docker 卷中，删除容器不会丢失数据 |
| **默认密码** | .env.example 中的密码仅用于本地开发，生产环境务必修改 |
| **WSA 兼容性** | Windows Subsystem for Android (WSA) 可能与 Docker Desktop 冲突 |
...
| 数据类型 | 备份位置 | 说明 |
|---------|---------|------|
| 应用数据 | Docker 卷 | `docker volume ls` 查看 |
| 配置文件 | .env 文件 | 包含密码等敏感信息 |
| 代码数据 | Git 仓库 | 定期提交 |
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| Docker 启动失败 | WSL2 未安装 | 运行 `wsl --install` |
| 端口被占用 | 其他服务占用了端口 | 停止相关服务或修改端口 |
| 内存不足 | Docker 消耗过多内存 | 降低 Docker 内存限制 |
| 镜像下载慢 | 网络问题 | 配置 Docker 国内镜像 |
